...
void DMTimerImpl::SetTimerInfo(uint64_t qwIDEvent,
                               const std::string &strTimerObjName) {
...
uint64_t DMTimerImpl::GetRunningTimerID() const { return m_qwRunningIDEvent; }

const std::string &DMTimerImpl::GetRunningTimerObjectName() const {
  return m_strRunningTimerObjName;
}

void DMTimerImpl::SetSpeed(uint32_t dwSpeed) { m_dwSpeed = dwSpeed; }
...
uint64_t DMTimerImpl::GetBootTime() {
  ...
  uint64_t qwPassedTime =
      static_cast<uint64_t>(dwRealPassedTime) * m_dwSpeed / 10000;
  m_qwTotalTickCount += qwPassedTime;
  return m_qwTotalTickCount;
}

CDMTimerElement *DMTimerImpl::FetchElement() {
  return m_oTimerElementPool.FetchObj();
}
void DMTimerImpl::ReleaseElement(CDMTimerElement *pElement) {
  m_oTimerElementPool.ReleaseObj(pElement);
}
bool DMTimerImpl::TimerPending(CDMTimerElement *pElement) {
  return pElement->m_stEntry.next != NULL;
}
...
void DMTimerImpl::AddTimerElement(CDMTimerElement *pElement) {
  ...
  if (idx < TVR_SIZE) {
    vec = m_tv1.vec + (qwExpires & TVR_MASK);
  } else if (idx < (1 << (TVR_BITS + TVN_BITS))) {
    vec = m_tv2.vec + ((qwExpires >> TVR_BITS) & TVN_MASK);
  } else if (idx < (1 << (TVR_BITS + 2 * TVN_BITS))) {
    vec = m_tv3.vec + ((qwExpires >> (TVR_BITS + TVN_BITS)) & TVN_MASK);
  } else if (idx < (1 << (TVR_BITS + 3 * TVN_BITS))) {
    vec = m_tv4.vec + ((qwExpires >> (TVR_BITS + 2 * TVN_BITS)) & TVN_MASK);
...
CDMTimerElement *DMTimerImpl::__GetTimerInfoByEntry(struct list_head *head) {
  return (CDMTimerElement *)((char *)head -
                             offsetof(CDMTimerElement, m_stEntry));
}
#if defined(__GNUC__) || defined(__clang__)
...
int DMTimerImpl::Run(void) {
  ...
  while (DM_TIME_NOT_EQ(m_qwCurTime, m_qwLastTime)) {
    ...
    if (!index && (!__Cascade(&m_tv2, DM_INDEX(m_qwLastTime, 0))) &&
...
bool DMTimerImpl::SetTimer(ITimerSink *pSink, uint64_t qwIDEvent,
                           uint64_t qwElapse) {
  return SetTimer(pSink, qwIDEvent, qwElapse, qwElapse, dm::any(), false);
}

bool DMTimerImpl::SetTimer(ITimerSink *pSink, uint64_t qwIDEvent,
                           uint64_t qwElapse, uint64_t qwFirst,
                           const dm::any &oAny, bool bExact) {
  if (!pSink)
    return false;
...
bool DMTimerImpl::SetTimerLambda(ITimerSink *pSink, uint64_t qwIDEvent,
                                 uint64_t qwElapse, uint64_t qwFirst,
                                 DMFunction fFun) {
  if (!pSink || !fFun)
    return false;

  // 修改点: 保持统一逻辑, 先删除可能存在的旧计时器
  KillTimer(pSink, qwIDEvent);
...
bool DMTimerImpl::SetTimerCron(ITimerSink *pSink, uint64_t qwIDEvent,
                               const std::string &strCron, DMFunction fFun) {
  if (!pSink || strCron.empty() || !fFun)
    return false;
